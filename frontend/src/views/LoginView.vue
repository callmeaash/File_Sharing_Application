  ...
  <div class="min-h-screen flex items-center justify-center bg-[var(--color-bg)] px-4">
    <div class="w-full max-w-md">
      ...
      <div class="text-center mb-10">
        ...
        <p class="text-sm text-[var(--color-text-muted)] mt-1">Sign in to your account</p>
      </div>

      <!-- Form -->
      <form @submit.prevent="handleLogin" class="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-8 shadow-sm">
      ...
      <p class="text-center text-sm text-[var(--color-text-muted)] mt-6">
        ...
        <router-link to="/register" class="text-[var(--color-text)] font-medium hover:underline">Create one</router-link>
      </p>
    </div>
  </div>
</template>
